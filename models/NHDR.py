...
class NHDRRNet(nn.Module):
    ...
    def _make_encoder(self, in_c, out):
        encoder = nn.Sequential(
            PaddedConv2d(in_c, out, self.config['encoder_kernel_size'], self.config['encoder_stride']),
    ...
    def _make_triple_pass_layer(self):
        return [PaddedConv2d(self.config['triple_pass_filter'], self.config['triple_pass_filter'], 1, 1),
                PaddedConv2d(self.config['triple_pass_filter'], self.config['triple_pass_filter'], 3, 1),
                PaddedConv2d(self.config['triple_pass_filter'], self.config['triple_pass_filter'], 5, 1),
                PaddedConv2d(self.config['triple_pass_filter'] * 3, self.config['triple_pass_filter'], 3, 1)]
    ...
    def forward(self,x):
        ...
        image1 = x[:, :self.c_dim*2, :, :]
        image2 = x[:, self.c_dim*2:self.c_dim*4, :, :]
        image3 = x[:, self.c_dim*4:self.c_dim*6, :, :]
        original_image = image2[:, 3:6, :, :]
        ...
        return torch.clamp(out+original_image, 0, 1)